...
import { STAGE_SHORT_LABELS, STAGE_ICONS } from '~/constants/taskCatalog'
...
// ── All projects visible to current PM ────────────────────────────────────────
const allProjects = computed(() =>
  auth.currentUser ? crm.projectsByPm(auth.currentUser.id) : crm.projects
)

// ── Determine current design stage of a project ───────────────────────────────
...
function getProjectStage(p: CrmProject): DesignStage | 'completed' {
  ...
  // All tasks done but status not completed → last active stage or fallback
...
const columns = computed(() => {
  return COLUMNS.map(col => ({
    ...col,
    projects: allProjects.value.filter(p => getProjectStage(p) === col.key),
  }))
})
    ...
    <header class="h-16 flex items-center justify-between px-6 bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800 flex-shrink-0">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-slate-400">{{ allProjects.length }} проектов по этапам</p>
        </div>
      </div>
        ...
        >
          ...
          <div class="flex-1 overflow-y-auto space-y-2.5 pr-0.5">
            ...
            <button
              ...
              :key="p.id"
              v-memo="[p.status, p.tasks.length, p.tasks.filter(t => t.status === 'done').length]"
              class="w-full text-left bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-3.5 shadow-sm hover:shadow-md hover:border-blue-200 dark:hover:border-blue-800 transition-all group"
            ...
            >
              ...
              <div class="flex items-start gap-2 mb-2">
                <div class="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 group-hover:bg-blue-50 dark:group-hover:bg-blue-950 transition-colors">
                  <UIcon :name="TYPE_ICONS[p.type]" class="w-3.5 h-3.5 text-slate-500 group-hover:text-blue-500" />
                </div>
              ...
              <div v-if="p.tasks.length > 0" class="mb-2">
                <div class="flex items-center justify-between text-xs text-slate-400 mb-1">
                  <span>{{ p.tasks.filter(t => t.status === 'done').length }}/{{ p.tasks.length }} задач</span>
              ...
              <div class="flex items-center justify-between gap-1.5 mt-2">
                <span class="text-xs font-medium text-slate-500">
                  ...
                  <span class="font-normal text-slate-400">сум</span>
                </span>
